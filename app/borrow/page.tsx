...
export default function BorrowPage() {
  ...
  const calculateBorrowAmount = () => {
    ...
    const collateralValue =
      Number.parseFloat(collateralAmount) * (collateralAssets.find((a) => a.symbol === collateralAsset)?.currentPrice || 0)
            ...
            <CardContent className="p-4 h-[500px] overflow-y-auto scrollbar-hide">
              <div className="space-y-2">
                {collateralAssets.map((asset, index) => (
                  <div
                    key={index}
                    className={`card-dark rounded-lg p-3 cursor-pointer transition-all hover:glow-purple ${collateralAsset === asset.symbol ? "border-purple-500/50 glow-purple bg-purple-500/5" : ""
                      }`}
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center space-x-3">
                        <img
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-lg ${collateralAsset === asset.symbol
                            ? ""
                            : `bg-[${asset.color}20]`
                            }`}
                          src={asset.icon}
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {debtAssets.map((asset) => (
                        <Button
                          key={asset.symbol}
                          variant="outline"
                          className={`token-selector-dark p-2 h-auto flex-col space-y-1 rounded-lg ${borrowAsset === asset.symbol ? "border-purple-500 bg-purple-500/10 glow-purple" : ""
                            }`}
